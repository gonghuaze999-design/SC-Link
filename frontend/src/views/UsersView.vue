  ...
  <div>
    <div class="bg-white rounded-xl border border-line">
      <div class="flex items-center gap-3 p-4 border-b border-line">
        <input
          v-model="keyword"
          class="w-64 border border-line rounded-lg px-3 py-2 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/20"
          placeholder="搜索账号/姓名"
          @keyup.enter="load"
        />
        <button
        ...
        <button class="ml-auto px-4 py-2 rounded-lg text-xs bg-primary text-white transition" @click="openCreate">
          + 新增用户
        </button>
      </div>
      <table class="w-full text-[13px]">
        ...
        <tbody>
          ...
          <tr v-for="u in users" :key="u.id" class="border-b border-line hover:bg-slate-50/60 transition">
            ...
            <td class="px-4 py-3 text-muted">{{ fmtTime(u.last_login_at) }}</td>
            <td class="px-4 py-3 text-right whitespace-nowrap">
              <button class="text-xs text-primary hover:underline mr-3" @click="openEdit(u)">编辑</button>
              <button
    ...
    >
      <div class="bg-white rounded-xl w-[420px] p-6 shadow-2xl">
        ...
        <div class="grid grid-cols-2 gap-3">
          <div>
            ...
            <input
              ...
              class="w-full border border-line rounded-lg px-3 py-2 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 disabled:bg-slate-50 disabled:text-muted"
            />
          </div>
          <div>
            <label class="block text-xs text-muted mb-1.5">姓名</label>
            <input
          ...
          <div>
            <label class="block text-xs text-muted mb-1.5">角色 *</label>
            <select
              v-model="form.role"
              class="w-full border border-line rounded-lg px-3 py-2 text-sm outline-none focus:border-primary bg-white"
            >
          ...
          <div>
            <label class="block text-xs text-muted mb-1.5">
              {{ dialog.mode === 'create' ? '初始密码 *(至少 8 位)' : '重置密码(留空不改)' }}
            </label>
            <input
              v-model="dialog.mode === 'create' ? form.password : form.new_password"
              type="password"
              class="w-full border border-line rounded-lg px-3 py-2 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/20"
            />
          </div>
          <div>
            <label class="block text-xs text-muted mb-1.5">电话</label>
            <input
              v-model="form.phone"
              class="w-full border border-line rounded-lg px-3 py-2 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/20"
            />
          ...
          <div>
            <label class="block text-xs text-muted mb-1.5">邮箱</label>
            <input
              v-model="form.email"
              class="w-full border border-line rounded-lg px-3 py-2 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/20"
            />
        ...
        <div class="flex justify-end gap-2 mt-5">
          ...
          <button
            class="px-4 py-2 rounded-lg text-xs bg-primary text-white transition"
            @click="submit"
          >
