  ...
  <div>
    <div class="bg-white rounded-xl border border-line">
      <div class="flex items-center gap-3 p-4 border-b border-line">
        ...
        <button class="ml-auto px-4 py-2 rounded-lg text-xs bg-primary text-white transition" @click="openCreate">+ 新增客户</button>
      </div>
      <table class="w-full text-[13px]">
        <thead>
          <tr class="text-left text-muted text-xs border-b border-line">
            <th class="px-4 py-3 font-medium">采购主体</th>
            <th class="px-4 py-3 font-medium">类型</th>
            <th class="px-4 py-3 font-medium">意向采购方式</th>
            <th class="px-4 py-3 font-medium">意向数量</th>
            <th class="px-4 py-3 font-medium">验资</th>
        ...
        <tbody>
          ...
          <tr v-for="c in rows" :key="c.id" class="border-b border-line hover:bg-slate-50/60 transition">
            ...
            <td class="px-4 py-3 text-muted">{{ c.customer_type || '—' }}</td>
            <td class="px-4 py-3 text-muted">{{ (c.intent_modes || []).join(' / ') || '—' }}</td>
            <td class="px-4 py-3 text-muted">{{ c.intent_quantity || '—' }}</td>
            <td class="px-4 py-3">
              <span class="text-[11px] px-2 py-0.5 rounded" :class="c.verified ? 'bg-green-50 text-green-600' : 'bg-amber-50 text-amber-600'">{{ c.verified ? '已验资' : '未验资' }}</span>
            </td>
            <td class="px-4 py-3">
              <span v-if="c.value_grade" class="text-[11px] px-2 py-0.5 rounded bg-blue-50 text-blue-600">{{ c.value_grade }} 级</span>
              <span v-else class="text-muted">—</span>
            </td>
            <td class="px-4 py-3 text-muted text-xs">{{ editorName(c.last_editor_id) }}<br>{{ fmt(c.updated_at) }}</td>
            <td class="px-4 py-3 text-right whitespace-nowrap">
              <button class="text-xs text-primary hover:underline mr-3" @click="detail = c">详情</button>
              <button class="text-xs text-primary hover:underline mr-3" @click="openEdit(c)">编辑</button>
              <button class="text-xs text-red-500 hover:underline" @click="remove(c)">删除</button>
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <div v-if="dialog.show" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50" @click.self="dialog.show = false">
      <div class="bg-white rounded-xl w-[720px] max-h-[86vh] overflow-y-auto shadow-2xl">
        ...
        <div class="p-6 grid grid-cols-2 gap-4">
          ...
          <div class="col-span-2 text-xs font-bold text-primary pt-2">交易意向</div>
          <div>
            <label :class="labelCls">意向采购方式(可多选)</label>
            <div class="flex gap-2">
              <button v-for="m in modeOptions" :key="m" type="button" class="px-3 py-1.5 rounded-lg text-xs border transition" :class="((form.intent_modes as string[]) || []).includes(m) ? 'border-primary bg-blue-50 text-primary' : 'border-line text-muted hover:border-slate-400'" @click="toggleMode(m)">{{ m }}</button>
            </div>
          </div>
          ...
          <div class="col-span-2 text-xs font-bold text-primary pt-2">客户画像</div>
