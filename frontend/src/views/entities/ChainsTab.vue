  ...
  <div>
    <div class="bg-white rounded-xl border border-line">
      <div class="flex items-center p-4 border-b border-line">
        <div class="text-xs text-muted">同一条海外链路方下可有多个国内供货方作为代表销售设备;删除前须先解除其下供货方的关联。</div>
        <button class="ml-auto px-4 py-2 rounded-lg text-xs bg-primary text-white transition shrink-0" @click="openCreate">+ 新增链路方</button>
      </div>
      <table class="w-full text-[13px]">
        <thead>
          <tr class="text-left text-muted text-xs border-b border-line">
            <th class="px-4 py-3 font-medium">链路方名称</th>
            <th class="px-4 py-3 font-medium">区域</th>
            <th class="px-4 py-3 font-medium">联系人</th>
            <th class="px-4 py-3 font-medium">联系方式</th>
            <th class="px-4 py-3 font-medium">背景备注</th>
        ...
        <tbody>
          ...
          <tr v-for="c in rows" :key="c.id" class="border-b border-line hover:bg-slate-50/60 transition">
            ...
            <td class="px-4 py-3 text-muted">{{ c.region || '—' }}</td>
            <td class="px-4 py-3 text-muted">{{ c.contact_person || '—' }}</td>
            <td class="px-4 py-3 text-muted">{{ c.contact_info || '—' }}</td>
            <td class="px-4 py-3 text-muted max-w-[220px] truncate" :title="c.description">{{ c.description || '—' }}</td>
            <td class="px-4 py-3 text-muted text-xs">{{ userNames[c.last_editor_id!] || '—' }}<br>{{ fmt(c.updated_at) }}</td>
    ...
    <div v-if="dialog.show" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50" @click.self="dialog.show = false">
      <div class="bg-white rounded-xl w-[440px] p-6 shadow-2xl">
        <div class="text-sm font-bold mb-4">{{ dialog.mode === 'create' ? '新增链路方' : `编辑:${dialog.target?.name}` }}</div>
