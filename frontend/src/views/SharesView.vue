...
<template>
  <div class="space-y-5">
    ...
    <div v-if="pendingForMe().length" class="bg-white rounded-xl border border-amber-200">
      <div class="px-5 py-3.5 border-b border-line text-sm font-bold text-amber-600">待我审批的共享申请</div>
      <div v-for="s in pendingForMe()" :key="s.id" class="px-5 py-3.5 flex items-center gap-3 border-b border-line last:border-0">
        <div class="flex-1">
          <div class="text-sm font-medium">{{ userName(s.requester_id) }} 申请与你共享:<b>{{ scopeText(s.scopes) }}</b></div>
          <div class="text-xs text-muted mt-0.5">{{ s.note ? `附言:${s.note} · ` : '' }}{{ fmt(s.requested_at) }} · 批准后共享范围内数据互相可见、共同维护</div>
        </div>
        <button class="px-5 py-2.5 rounded-lg text-[13px] bg-primary text-white" @click="respond(s, 'approve')">批准</button>
        <button class="px-5 py-2.5 rounded-lg text-[13px] border border-line text-muted" @click="respond(s, 'reject')">拒绝</button>
      </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-line">
      <div class="flex items-center px-5 py-3.5 border-b border-line">
        ...
        <button class="ml-auto px-5 py-2.5 rounded-lg text-[13px] bg-primary text-white" @click="dlg.show = true">+ 发起共享申请</button>
      </div>
      <table class="w-full text-sm">
        <thead>
          <tr class="text-left text-muted text-[13px] border-b border-line">
            <th class="px-5 py-3.5 font-medium">对方</th>
            <th class="px-5 py-3.5 font-medium">方向</th>
            <th class="px-5 py-3.5 font-medium">共享范围</th>
            <th class="px-5 py-3.5 font-medium">状态</th>
            <th class="px-5 py-3.5 font-medium">申请时间</th>
            <th class="px-5 py-3.5 font-medium">附言</th>
        ...
        <tbody>
          ...
          <tr v-for="s in shares" :key="s.id" class="border-b border-line hover:bg-slate-50/60 transition">
            <td class="px-5 py-3.5 font-medium">{{ userName(s.requester_id === auth.user?.id ? s.target_id : s.requester_id) }}</td>
            <td class="px-5 py-3.5 text-muted">{{ s.requester_id === auth.user?.id ? '我发起' : '对方发起' }}</td>
            <td class="px-5 py-3.5 text-muted text-[13px]">{{ scopeText(s.scopes) }}</td>
            <td class="px-5 py-3.5"><span class="text-xs px-2 py-0.5 rounded" :class="statusMeta[s.status]?.cls">{{ statusMeta[s.status]?.label }}</span></td>
            <td class="px-5 py-3.5 text-muted text-[13px]">{{ fmt(s.requested_at) }}</td>
            <td class="px-5 py-3.5 text-muted text-[13px] max-w-[160px] truncate" :title="s.note">{{ s.note || '—' }}</td>
            <td class="px-5 py-3.5 text-right">
              <button v-if="s.status === 'active'" class="text-[13px] text-red-500 hover:underline" @click="cancel(s)">解除共享</button>
              <span v-else-if="s.status === 'pending'" class="text-[13px] text-muted">等待对方处理</span>
            </td>
          </tr>
        </tbody>
      </table>
      <div class="px-5 py-3.5 border-t border-line text-xs text-muted">
    ...
    <div v-if="dlg.show" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50" @click.self="dlg.show = false">
      <div class="bg-white rounded-xl w-[440px] p-6 shadow-2xl">
        <div class="text-base font-bold mb-4">发起数据共享申请</div>
        <div class="grid gap-3">
          ...
          <div>
            ...
            <div class="flex gap-2 flex-wrap">
              <button type="button" class="px-3 py-1.5 rounded-lg text-[13px] border transition" :class="form.scopes.includes('all') ? 'border-primary bg-blue-50 text-primary' : 'border-line text-muted'" @click="toggleScope('all')">全部数据</button>
              <button type="button" class="px-3 py-1.5 rounded-lg text-[13px] border transition" :class="form.scopes.includes('supplier') ? 'border-primary bg-blue-50 text-primary' : 'border-line text-muted'" @click="toggleScope('supplier')">上游供货方</button>
              <button type="button" class="px-3 py-1.5 rounded-lg text-[13px] border transition" :class="form.scopes.includes('customer') ? 'border-primary bg-blue-50 text-primary' : 'border-line text-muted'" @click="toggleScope('customer')">下游客户</button>
              <button type="button" class="px-3 py-1.5 rounded-lg text-[13px] border transition" :class="form.scopes.includes('middle') ? 'border-primary bg-blue-50 text-primary' : 'border-line text-muted'" @click="toggleScope('middle')">中间层</button>
            </div>
          </div>
        ...
        <div class="flex justify-end gap-2 mt-5">
          ...
          <button class="px-5 py-2.5 rounded-lg text-[13px] bg-primary text-white" @click="submit">提交申请</button>
        </div>
      </div>
    </div>
  </div>
